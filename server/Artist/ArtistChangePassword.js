...
const ArtistChangePassword = async ({ username, currentPassword, newPassword }) => {
    return new Promise((resolve, reject) => {
        ...
        db.getConnection()
            .then((connection) => {
                connection.query(
                    "SELECT password FROM artists WHERE username = ?",
                    [username],
                    (error, results) => {
                        if (error) {
                            connection.release();
                        ...
                        connection.query(
                            "UPDATE artists SET password = ? WHERE username = ?",
...
module.exports = { ArtistChangePassword };