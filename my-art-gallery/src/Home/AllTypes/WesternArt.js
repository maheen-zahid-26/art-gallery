...
const WesternArt = () => {
  const [westernArts, setWesternArts] = useState([]);

  useEffect(() => {

    console.log("Fetching Western artworks...");

    fetch('http://localhost:9000/homePage/getArtworksByType?Type=Western Art')
    .then(response => {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>Western Art</h2>
      <div style={styles.grid}>
        {westernArts.length > 0 ? (
          westernArts.map((artwork) => (
          ...
          <p>No western artworks available.</p>
        )}
      </div>
    </div>
  );
